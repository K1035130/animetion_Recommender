...
def answers(client):
    ...
    items = client.get("/questionnaire", params={"n": 6}).json()["items"]
...
def test_questionnaire_folds_and_orders(client):
    items = client.get("/questionnaire", params={"n": 20}).json()["items"]
    assert len(items) == 20
...
def test_recommend_shape_and_order(client, answers):
    b = client.post("/recommend", json={"answers": answers, "top_k": 10}).json()
    # skip 不产生记录
...
def test_anime_detail(client):
    d = client.get("/anime/328609").json()          # 孤独摇滚！
