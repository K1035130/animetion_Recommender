...
# 📌 **元信息章节不参与 heimu 剧透门控**（2026-08-23）。
#
# 🚨 触发案例《神枪少女》：「衍生作品 > 动画版」那条 176 字**全是播出日期、
#    话数、制作阵容**，一句剧情都没有，却因为末句吐槽「表示对1期的风格不怎么
#    感冒」里有 14 字 heimu（8.0%）被整条判为剧透 ⇒ 默认路径下唯一含播出时间
#    的语料被门控挡掉，问「什么时候播出的」答不出来。
#
# ⚠️ **为什么不改成按占比设阈值**（那才是下面那行注释原本描述的做法）：
#    抽样 60 页的 heimu 实测是**混合用途** —— 真剧透（「实际是葵的亲生父亲，
#    与世良曾是恋人」「当然，这位是万年卧底」）与纯吐槽（「凑字的」「社会我
#    安哥，人狠话不多」「岸波白野：石川由依」）混在一起，而**真剧透往往只占
#    几个字**。按占比放行会成片放出真剧透，与 B.4「rerank 分噪声 1e-3 所以
#    绝对地板调不稳」是同一类错误：拿一个与目标无关的量当判据。
#
# ⇒ 改用**章节语义**这个与目标直接相关的信号：衍生作品／出版信息／相关音乐
#   这类章节按定义就不讲剧情，里面的 heimu 必然是吐槽。
#   实测只影响 **413 条**（占全部剧透标注的 2.1%），精准且风险极低。
# ⚠️ **剧透框（SPOILER_BOX）不豁免** —— 它标的是整段剧情概要，编者明确声明
#    「以下内容含有剧透」，出现在元信息章节里也该当真。只豁免 heimu 那一支。
META_SECTION = re.compile(
    r"衍生作品|出版信息|相关音乐|主题歌|发行|各话|制作人员|职员表|STAFF"
    r"|跨媒体|媒体展开|商品|周边", re.IGNORECASE)
    ...
    for sec in doc.xpath(".//section[@data-mw-section-id]"):
        ...
        for text, hm, boxed in made:
            ...
            out.append({
                ...
                "spoiler_box": boxed,
                # ⚠️ 元信息章节豁免 heimu 那一支，但**不豁免剧透框**（见 META_SECTION）。
                "spoiler_level": 1 if (boxed or (
                    hm > 0 and not META_SECTION.search(path or ""))) else 0,
            })
