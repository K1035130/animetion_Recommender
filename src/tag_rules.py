...
RE_YEAR = re.compile(
    ...
    r"|\d{4}年\d{1,2}月新番"
    r"|\d{4}里番"               # 2019里番
    r"|\d{2}年代"               # 90年代 / 80年代（取消年份下限后大量出现）
    r")$"
)
...
FORM = {
    ...
    "独立剧场版", "剧场版动画", "OVA动画", "TV动画", "实验动画",
    # 繁体变体（老条目的编辑者用繁体的比例明显更高）
    "劇場版", "短編", "動畫片", "剧集",
}
...
REGION = {
    ...
    "中国原作改编", "韩国原作改编",
    # 2026-08-11：把 meta_tags 也过一遍本管线时暴露的漏网。
    # 地区进题材向量的后果和「童年」一样 —— 所有苏联动画会仅因产地而互相相似。
    "香港", "台湾", "加拿大", "欧洲", "俄罗斯", "苏联", "捷克", "美剧",
}

# 制作公司 / 发行方 / 厂牌（含里番厂牌）
STUDIO = {
    # ---- 2026-08-11 自动检出（方法同 STAFF 处注释）----
    # 「魔人」看着像题材词，但打它的 33 部作品 100% 都有同名社团参与，
    # 是同人厂牌不是题材。「上美」是上海美术电影制片厂的缩写。
    # ⚠️ chippai 同样是 10/10 命中，但它在 ASCII_GENRE 白名单里（本文档第 13 节
    #    把它列为 13 个题材缩写之一）。df 仅 10，且「以该题材命名的社团」完全
    #    可能同时成立，样本太小不足以推翻已决事项 —— 维持判为题材词。
    "魔人", "東映", "上美", "迪斯尼", "小丑社", "东京movie",
    # 日本主要动画公司
    ...
    # 取消年份下限后新增
    "米高梅", "奶油柠檬", "华纳", "环球", "米拉麦克斯", "AnimeInternational",
    "虫Production", "虫プロダクション", "东京电影新社", "日升",
    "葦プロダクション", "日本サンライズ", "スタジオぴえろ", "東京ムービー",
}

# 声优 / 监督 / 脚本 / 音乐 —— 走 AniList staff 结构化字段
STAFF = {
    # ---- 2026-08-11 自动检出，非手工枚举 ----
    # 方法：拿 dump 的 person.jsonlines（9 万人名）与词表求交集，再用
    # subject-persons ∪ person-characters 交叉验证 ——「若 tag T 真是某人的
    # 名字，打了 T 的作品应大量真有此人参与」。命中率 >=50% 判为人名。
    # 这一步同时排除了同名巧合：「青春」(0%/160部)、「萌」(1%/234部)、
    # 「少女」「料理」在 person 表里都有同名条目，但作品与人零关联，是真题材词。
    # ⚠️ 声优必须走 person-characters（作品→角色→声优两跳），
    #    只查 subject-persons 会把中井和哉、坂本真綾 全部漏掉。
    "田中公平", "梶島正樹", "小畑健", "高畑勲", "石黒昇", "會川昇",
    "小林七郎", "小中千昭", "大張正己", "大平晋也", "出﨑統", "天野喜孝",
    "原恵一", "高橋留美子", "西久保瑞穂", "水島努", "恩田尚之", "今西隆志",
    "福田己津央", "百瀬義行", "湯浅政明", "渡部高志", "川井憲次", "北条司",
    "出崎哲", "乱交太郎", "中村隆太郎", "阿部記之", "西尾大介", "川越淳",
    "山本暎一", "前田俊夫", "佐藤順一", "今川泰宏", "朴璐美", "杉野昭夫",
    "中井和哉", "板野一郎", "坂本真綾", "三木眞一郎", "神谷明", "伊藤静",
    "平野俊貴", "吉野裕行", "川澄綾子",
    # 人工补充：命中率偏低但确系人名。小林清志是鲁邦三世的次元大介配音，
    # 标他的作品未必都把他登记在 staff 里。
    "小林清志",
    # ---- 第二轮：共现检测抓到、但因繁简字形差异过大而无法自动确认 ----
    # 冨樫義博↔富坚义博、あだち充↔安达充 这类字符重合率不到 20%，
    # 靠字面相似度判不出来（要彻底自动化得引 opencc 做繁简转换，
    # 为这个规模不值得）。以下经人工核对，共现率均 100%。
    "林太郎", "富坚义博", "石黑升", "安达充", "峰仓和也", "高桥直人",
    "金泽胜真", "百濑义行", "鹤卷和哉", "五武冬史",
    # 声优
    ...
    "钉宫理恵", "铃木乃",
    # 取消年份下限后新增：老一辈监督 / 原作者 / 声优
    "宫崎骏", "宮崎駿", "高畑勋", "手塚治虫", "藤子・F・不二雄", "松本零士",
    "望月智充", "出崎统", "梅津泰臣", "会川升", "井上喜久子", "川尻善昭",
    "查克琼斯", "真下耕一", "川澄绫子", "山内重保", "大张正己", "安彦良和", "今敏", "大友克洋", "押山清高", "宇多田光",
    "菊地康仁", "锦织博", "长浜博史", "细田直人", "佐藤竜雄", "石黑昇",
    "永野护", "美树本晴彦", "结城信辉", "北爪宏幸", "逢坂浩司",
    "久石让", "久石讓", "森本晃司", "高橋良輔", "湯山邦彦", "林原惠美",
    "四月一日", "近藤喜文", "宫崎吾朗", "米林宏昌",
}

# 作品名 / IP —— 会让同系列相似度虚高，不是口味特征
IP = {
    "金田一",  # 金田一少年事件簿。作品名不是口味特征
    # ---- 2026-08-11 共现检测抓出的作品名（打此 tag 的作品高度共享同一原作者/版权方）----
    "死神", "变形金刚", "天地無用", "天地无用", "哆啦A夢", "装甲骑兵",
    "世界名作剧场", "棒球大联盟", "福星小子", "秀逗魔导士", ".hack系列",
    "最游记", "盖塔", "机动警察", "洛克人", "魔炮", "魔法少女奈叶",
    "亚尔斯兰战记", "最终幻想", "飞出个未来", "初音岛", "樱花大战",
    "向阳素描", "克隆人战争", "黑塔利亚", "勇者シリーズ", "全金属狂潮",
    "数码兽", "寒蝉鸣泣之时", "辛普森一家",
    ...
    # 取消年份下限后新增
    "ルパン三世", "ドラえもん", "セーラームーン", "机器猫", "圣斗士星矢",
    "北斗神拳", "城市猎人", "灌篮高手", "犬夜叉", "浪客剑心", "幽游白书",
    "百变小樱", "棋魂", "钢之炼金术师", "反叛的鲁鲁修",
    "数码暴龙", "乱马", "圣斗士", "童梦", "阿基拉", "攻壳", "麻辣教师",
}
...
META = {
    ...
    "退队流", "伪",
    # 取消年份下限后新增的一整类：怀旧情绪 / 观看状态。
    # ⚠️「童年」在 2011+ 口径下 df=257，全年份下暴涨到 971 —— 它是怀旧标记，
    #   不是题材。老番几乎人人打，进 tag 向量会让所有老番互相相似。
    "童年", "童年回忆", "儿时回忆", "童年系", "童年系列", "早期优秀作品",
    "补旧番", "旧物", "回忆", "怀旧", "情怀",
    "烂尾", "没看全", "没看完", "沒看全", "沒看完", "内涵", "有生之年系列",
    "其他", "崩坏", "电视",
    # 2026-08-11：观看状态与怀旧的繁体/变体写法，同上理由
    "已下载", "已删除", "未看完", "没看完系列", "未定档", "旧番",
    "童年回憶", "經典", "崩",
}
...
# Python 的 set 字面量已经去重了。那类问题交给 ruff（B033），已在依赖里：
#     uv run ruff check src/ scripts/
# 补完规则记得跑一次。实测它一次抓出 10 处，比人眼可靠。
