...
⚠️ 根因：`parse_moegirl.py` 的 `SPOILER_BOX` 正则只认
`以下内容含有剧透成分`，而**萌娘另有一套更常用的模板 `剧透提醒`**（原始 HTML
字符串计数：≥200 页用前者的写法，且模板字样会漏进正文）。
...
⚠️ **但别把这条理解成「门控可以不做」** —— `spoiler_level=1` 的那 2,418 条
（12.4%）仍然挡住，漏的是没被识别的那部分。

⬜ 真要修的话在 `SPOILER_BOX` 加 `剧透提醒|剧透警告|剧透注意`，几行的事，
但**必须在重新编码之前做** —— 改 chunk 文本就是换缓存键，等于重新花钱请求 API。
