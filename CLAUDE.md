...
**语料层全部完工，且已全部转为中文（H 节）。下一步：阶段 05 检索层**，
完整设计见 **G 节**。LLM 选型已定：`PRIMARY = Qwen/Qwen3-14B`（G.5b · G.5e）。

⚠️ **实现顺序有变（2026-08-18）**，三条都是实测逼出来的：
① 语料转中文后 **HyDE 在三条查询上全都不如「原查询直接检索」**，降级成可开关的
第二条腿走 RRF（**G.5d**）；② **真正的瓶颈是检索不是模型** —— 四道问答测试里三道
败在"角色本人的 chunk 没被召回"，所以 **G.4 的 alias 消歧路径要排在 HyDE 前面**（**G.5g**）。
③ **`LIMIT 5/6` 太小**，实测目标 chunk 常在第 9–38 位 ⇒ 定案
**召回 50 → `bge-reranker-v2-m3` → 前 8**，纯向量前 8 是 0/5、加 rerank 是 5/5（**G.6**）。
✅ LLM 已填：`PRIMARY = Qwen3-14B` · `FALLBACKS = (GLM-4.5-Air,)`（**G.5f**）。
⬜ 悬着一件事：**路径③ 跨作品检索 579 ms**，要不要建 HNSW —— 见 F.1 的标注。

### ▶️ 下次开工从这里接（2026-08-18 收工）

**阶段 05 检索层，实现顺序已由实测定死：**

```
① alias 消歧      名字 → alias → character_id → 按 id 直取     G.4 · G.5g
② 基线召回        作用域内向量 top-50（角色 40 + 系列 10 保底）  G.6
③ rerank          BAAI/bge-reranker-v2-m3 → 前 8               G.6
④ 交给 LLM        llm.answer()，chunks 为空则短路不调用          G.4 状态③
⑤ （可选）HyDE    第二条腿走 RRF，默认关                        G.5d
```

⚠️ **① 排在最前面不是随意排的**：四道问答测试里三道败在「角色本人的 chunk 没被召回」，
而根因是角色 chunk 正文里没有自己的名字（F.7b ①）—— 加大 k 只是碰运气。

⬜ **动手前先补一轮 10–20 条查询的正式对照**，判定 HyDE 该不该默认开（G.5d）。
⬜ **顺带重测跨语言惩罚**：日文已降到 0.36%，原先那个 −27.7pp 重跑一次就是
第 5 周报告的现成材料。

**未提交**（Kevin 自己提交）：`CLAUDE.md` · `src/llm.py`
—— 翻译管道那批已在 `7ce0462`…`158dd90` 四个提交里入库。
...
| **4** | 萌娘语料 ✅ · 角色语料(03) ✅ · 语料转中文 ✅ · LLM 选型 ✅ · **检索层(05) ⬜** | 🔄 **← 下一步在这** |
| **5** | **离线评测（核心卖点，不可压缩）** | ⬜ |
| 6 | 信息增益选题 + 账号系统 + 季度同步 | ⬜ |
...
| **05** | 检索层（流程 B + C） | ⬜ | 依赖 02·03，设计见 **G 节** |
...
## G. 检索层设计（阶段 05，2026-08-16 定，⬜ 未实现）
...
Neon 缩到零唤醒的只是数据库 compute。⬜ **先量真实冷启动延迟再决定**，别先付钱。
详见第 5 节「已升级付费计划」。
