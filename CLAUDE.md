...
测试 **284 项**（18 打分一致性 + 24 接口 + **69 检索层** + **28 解析器** + 17 声优
+ **54 路由** + 5 客户端收尾 + **16 LLM 分类式调用**（find_gate/classify_intent）
+ 4 找番管道 + **12 意图校验路由 wiring** + **37 账号系统**）。
...
+ `src/ratings.py` + `src/quota.py` + 7 个端点 + 前端 `session.tsx`。测试 **37 项**。
...
「邮箱不存在」与「密码错误」报同一句话是基本操作，但**只统一文案不够**：
邮箱不存在时直接返回会比密码错快几十毫秒（少跑一次**故意很慢**的 argon2），
掐表照样能枚举账号。⇒ 邮箱不存在时也拿 `auth.DUMMY_HASH` 跑一次校验。

#### ⚠️ 已知缺口（都是有意接受的，不是遗漏）

```
邮箱验证      不做（设计文档 §4 已定）⇒ unique 只防重复注册，
              不代表这个邮箱真属于注册者
