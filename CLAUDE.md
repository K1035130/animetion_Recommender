...
🚨 「纯按热度排序」实测更差，已否决   钉宫理惠「神乐」从第 3 掉到第 21
                                   ⇒ 改用 fav_done × 役别权重
🚨 voice 延迟 0.3 s → 26–37 s        数量级变化，⬜ 要不要换小模型待 Kevin 定
🚨 前端 hasStructured 曾把 answer 藏掉  改后端行为要回头查前端的旧优化
```

⬜ **等 Kevin 拍板的一件事**：voice 现在和流程 C 同一个延迟档
（瓶颈是 Qwen3-14B 约 8 token/s）。要压下去只有「给 voice 单配一个
8B 级模型」或「上流式输出」两条路，前者与 G.5f「fallback 按质量选不按延迟」
取向相反 —— 见第四部分那节末尾。
...
三处一起改之后：**132~186 字 / 25.8~36.5 秒，3/3 都是连贯段落，没有再抄表**。
...
🚨 **延迟代价（需要 Kevin 拍板要不要接受）**：

```
voice 分支   0.3–0.4 s（零模型）→ **意图校验 + 一次生成 ≈ 26–37 s**
```

这是**数量级变化**，与 intent 校验那次（0.3 → 4.9 s）不是一回事 ——
voice 现在和流程 C 同一档了。瓶颈是 Qwen3-14B 的吐字速度（实测约
8 token/s），不是资料量。⬜ 想压下去只有两条路：
① 给 voice 单独配一个更小更快的模型（任务简单：读一张表写一段话，
   8B 大概率够，`INTENT_PROVIDER` 已有现成的 Provider 写法）——
   ⚠️ 但这与 G.5f「fallback 按问答质量选不按延迟」的取向相反，要 Kevin 定；
② 流式输出（改动面大，前端也要动）。
...
```
prompts      871d63553987ad3e → 76ca3356f85565ab
fingerprint  （Qwen3-14B）    → 93844c79d07b3779
```
`prompt_digest()` 现在覆盖**四条** prompt。⚠️ 加这一行必然让历史评测数字
