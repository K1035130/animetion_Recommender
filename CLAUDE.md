...
- 按分钟限流，5,000 部分页拉约 100 次请求，加 sleep
...
- **不建 HNSW 索引** — 5,000 条暴力算余弦仅几毫秒，且是精确检索，比近似索引召回更准
...
一部动画一个 profile 向量用于**发现**；章节级 chunk 用于**剧情问答**。两者用途不同，维度也没理由统一——profile 只有 5,000 条，1024 维才 20 MB，完全不构成压力；真正吃存储的是 10 万条 chunk。
...
| anime_profile（1024 float4，**7,112 条**实测） | 29 MB |
| 元数据 + BM25 索引 | 15 MB |
...
💡 **jieba 自定义词典直接复用清洗后的 682 个 tag 词表。**「轻小说」「异世界」「泡面番」「萝卜」这类动画圈专有名词通用词典会切错，而它们全在 tag 词表里，零额外成本。
...
### 目的
先把 5,000 部聚成 N 个簇，每簇选一部代表作进问卷 → 用最少的题覆盖最大的口味空间。
...
| 批次 | 时间 | 内容 | 量级 |
|---|---|---|---|
| 1 | 第 1–2 周 | `anime_profile`，纯 Bangumi + AniList | **7,112 部**（实测）/ 29 MB |
...
**目标：拿到 5,000 部动画的干净结构化数据并落库。不碰 embedding，不碰萌娘百科，不碰前端。**
...
**实测底数（2026-08-10）：** 候选集 7,112 部共 **7,624 个不同 tag**，每部平均 10.9 个（中位 11）。

**① 「保留 300–800 个有效 tag」这个目标不成立，已作废**

实测：Bangumi 动画的**题材概念总量就是 200 个左右**。阈值 ≥8 得 200 个，≥10 得 180 个——差 20 个而已。想凑到 300–800 只能靠把噪声放进来，那是拿 P0 精度换数字好看。

**采用 `doc_freq >= 8` → 200 个题材 tag。**（`doc_freq` = tag 出现在多少部作品中，非累计投票数）

⚠️ **阈值从来不是关键变量，规则完整性才是。** 同样是 df≥8，规则不全时 KEEP 有 324 个（混着 124 个漏网人名/公司/IP），规则补全后是 200 个。调阈值的收益远小于补规则。

**② 分流结果（不是「保留 vs 丢弃」，是四个去处）**

| 类别 | 个数 | 去处 |
|---|---|---|
| **KEEP** | **200** | → 进 tag 向量，P0 用 |
| STUDIO | 195 | → 分流，作第 4 周 AniList `studios` 的交叉验证 |
| STAFF | 121 | → 分流，作 AniList `staff` 的交叉验证 |
| YEAR | 116 | 丢弃（`air_year` 列已有） |
| IP | 99 | 丢弃（作品名不是口味特征，且让同系列相似度虚高；系列关系走 `subject-relations`） |
| FORM | 46 | 丢弃（`form` 列 + meta_tags 已有） |
| META | 27 | 丢弃（`惨遭动画化` `作画崩坏` `虐狗` `爽文` 这类） |
| REGION | 17 | 丢弃（meta_tags 已有） |

**制作公司 195 个、人名 121 个——比题材 tag 加起来还多。** 这是 Bangumi tag 系统最大的特点，也是不做分流就直接算余弦会失真的原因：`ufotable` 和 `奇幻` 在向量里权重相同，显然不对。
...
```
漫画改(2,611) ← 漫改              最大的一对，覆盖 37% 候选集
游戏改(816)   ← GAL改, 手游改, 游戏改编, 游改
里番(803)     ← 2D里番, 里
轻小说改(761) ← 轻改
泡面番(827)   ← 泡面        治愈(498) ← 治愈系      萌(171) ← 萌系
18禁(277)     ← 18X, H      卖肉(206) ← 肉, 肉番    运动(197) ← 体育
无码(63)      ← 無修正      重口(49)  ← 重口味      腐向(29)  ← 腐
```
...
- [x] ~~真实候选集规模~~ → **7,112 部**，profile 向量 29 MB，总预算 299 MB / 余量 40%
...
- `ingest_raw`（原始 API 响应留档）：7,112 部 × 3 源 × 5–20 KB ≈ **100–400 MB**
- `rating_snapshot`（时序快照）：每周 × 3 源 × 7,112 部 × 一年 ≈ 110 万行 ≈ **220 MB**

现有预算已占 299 MB / 500 MB，**余量只有 200 MB**。两者叠加必然超限，而 Neon 免费层超限是**直接挂起项目**，不是计费。
