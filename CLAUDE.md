...
**库占用 45 MB / 500 MB。** 分词词典指纹 `bac2cf03a6c828a5`（第 3 周查询层启动时必须比对）。
...
三者都幂等，可任意顺序重跑。已实测：重跑不改行数、不洗掉别的脚本填的列。
...
### ⬜ 问卷选题必须排除直接续作（第 5–6 周，第 9 节实现时做）
...
### ⬜ P0 的 tag 向量要用 `meta_tags` 兜底（第 2 周）

**139 部（1.2%）清洗后 `tags` 为空**，其中包括化物语（done=37,573）。
...
**这 139 部 100% 都有 `meta_tags`**（化物语 = `奇幻`+`小说改`）。`meta_tags` 已是独立一列，P0 读两列合并即可，不必改 ETL。
另外全库清洗后平均只有 **4.6 个** tag/部，tag 向量比预想稀疏得多。
...
真实已用 **45 MB**，与原表「元数据+BM25 索引 20 MB」的估计差一倍多。

⚠️ **MVCC 膨胀会虚增占用。** 每轮批量 UPDATE 都会留下旧行版本：三轮回填后库从 43 MB 虚涨到 99 MB，
`VACUUM FULL` 后回到 45 MB。**批量回填之后记得跑一次**，否则会误判预算已经吃紧
...
💡 **jieba 自定义词典直接复用清洗后的 418 个 tag 词表。**「轻小说」「异世界」「泡面番」「萝卜」这类动画圈专有名词通用词典会切错，而它们全在 tag 词表里，零额外成本。
...
- [x] ~~真实候选集规模~~ → **11,453 部**（全年份）。灌库后实测占用 **45 MB**，加上未来的 vec 与 plot_chunk 总预算 ~344 MB / 余量 31%（vec 改 halfvec 则 ~320 MB / 36%），明细见第 5 节
