...
### ▶️ 下次开工从这里接（2026-08-19 第二轮收工）

✅ **`GET /api/season` 已上线**（单一入口路由的第 1 步）——
窗口口径的唯一定义处是 `recommend.cour_window()`（本季起点 −7 天 ~ 下季起点，右开），
实测与第四部分那次测量**逐条吻合**：2016 夏季 134 部、头名《你的名字。》。
✅ **prompt 已纳入 `llm.descriptor()` 指纹**（评测前必做的那条，见下方 ✅ 标注）。
**下一步：第 5 周离线评测 —— 不可压缩的主线，别再往前插新功能。**
...
⚠️ **两个修法都已实测但都还没实现**，且必须一起上（只修召回仍会被地板砍）。
...
`src/recommend.py`（cour_window）· `src/llm.py`（指纹）·
`server/main.py` + `server/schemas.py`（/api/season）· `tests/`（+8 项）
—— 阶段 05 那批在 `3e29085` 及其后，**文档拆分在 `838af6b`**（docs/ 已入库）。
...
测试 **60 项**（18 项打分一致性 + 19 项接口 + 23 项检索层/请求路径纯函数）。
