...
**0. 改列类型 `vector(1024)` → `halfvec(1024)`**（先做，10 分钟）
省 23 MB（47 → 23.5 MB），fp16 对余弦的影响可忽略。现在改是一条 ALTER，
灌完 11,453 条再改要重灌。新建 `sql/003_vec_halfvec.sql`。

**1. 摸清 embedding API 的行为**（10 分钟，见 A.7）
不是「本地 vs API 比对」——那个方案已废弃。要测的是：
① instruct 前缀是否自动加、能否关；② 批内数值稳定性。

**2. 建 embedding 缓存层**（见 A.7）
`hash(text + model + dim)` → npy/parquet，本地磁盘，不进 Neon。
⚠️ 这不是优化项，是第 4 周能否迭代 chunk 切分策略的前提。

**3. 编码 11,453 条 summary → `anime_profile.vec`**
⚠️ 589 部 summary 为空（5.1%），要决定怎么处理——留 NULL 还是用
`name + tags` 拼一个兜底文本。留 NULL 的话这批在向量检索里永远召不回，
与第 13 节那 142 部零 tag 向量的作品**大概率不是同一批**，值得先交叉看一下。
...
瓶颈不是请求数而是 TPM。28 M token 在 TPM=100 K 下约 4.7 小时、
500 K 下约 56 分钟、1 M 下约 28 分钟。**一次性跑通完全可行。**

成本也不是问题：真掉出免费层，按阿里云百炼 0.5 元/百万 token 算，
**全量重建一次 14 元**。

#### ⚠️ 真正的代价是迭代成本，不是一次性成本

RAG 的 chunk 切分策略（粒度、重叠、要不要带 section 标题前缀）**没有一次调对的**，
通常要 3–5 轮，每轮都要全部重新 embed：

| | 一次重建 | 5 轮迭代 |
|---|---|---|
| 本地（RTX 4050） | 15–30 分钟 | 免费，随便试 |
| API | 1–5 小时 | 烧掉整周 + 配额 |

**所以必须配缓存层**（见下），否则第 4 周的工作方式会从「改完就重跑看效果」
退化成「想清楚再跑，跑错了肉疼」—— 而那恰恰是个需要试错的环节。
...
不是「本地 vs API 比对」（那个方案已废弃），是**摸清你唯一那条路径的行为**：

1. **instruct 前缀**：Qwen3 的 query/document 是非对称编码。API 是否自动加前缀、
   能否关掉？如果是不可控的黑盒，你就失去了对这个非对称性的控制。
2. **批内数值稳定性**：把同一条文本塞进两个**不同组成**的 batch，对比结果。
   差异超过 1e-6 就说明「重建」不是 bit-identical 的，缓存层的复现承诺要打折扣。
...
按 0.5 元/百万算，10,000 次线上检索是 **1.15 元**。为这个建一套多模型切换系统，
工程量远超收益，何况它还不 work。

⚠️ **先搞清楚硅基流动给的是哪种免费**：是「固定 token 额度用完就停」，
还是「免费但限速（RPM/TPM）」？后者的失败模式是**变慢**不是**停服**，
处理方式完全不同（退避重试 vs 切换）。⬜ 未核实。
